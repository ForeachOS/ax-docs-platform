...
We're using a try-with-resources approach to ensure that every gets cleaned up nicely afterwards.
...
This code compiles as `InternalComponentOne` is a public class, but if you run the test, it fails with exception:
 ...
 No qualifying bean of type 'com.example.demo.modules.one.InternalComponentOne'
...
Since there is no dependency based ordering, the registration order will kept, causing the test to fail.
...
Beans annotated with `@Service` are by default exposed.
...
Remove `@AcrossDepends` from the `ModuleThree` class, and replace the `@Autowired(required=false)` by `@RefreshableCollection`:
...
This is might seem obvious with the above example, but let's shift the order around:
